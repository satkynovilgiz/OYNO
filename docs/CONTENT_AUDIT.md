...
## 8. Long-form localization 2.0 (2026-09-27)

**Schema before:** one Kyrgyz text column per field (`culture_items` title +
11 body fields + alt_names, `culture_materials` title/description/body,
`explore_regions.facts[]`, `quests` title/subtitle/cta_label). Already
per-language: region names, discovery titles, quest-step titles,
`culture_items.simple_summary_kg/ru/en` (RU/EN empty).

**Schema after:** `public.content_translations (content_type, content_id,
language, field, value, status)` - RU/EN only, reviewed rows public,
drafts hidden, admin-only writes (role-checked, audited). Kyrgyz rows are
untouched and stay canonical. Migrations:
`20260927000001_content_translations.sql` (table + admin RPCs) and
`20260927000002_content_translations_batch1.sql` (276 rows, generated by
`scripts/build-content-translations.py` from `content/translations/`).
**Both are unapplied** until run on the Supabase project.

**Resolver:** `src/services/content/localizedContent.ts` - requested
language → Kyrgyz, all-or-nothing per article body (never a mixed-language
article), with an internal status `available` / `fallback_to_kg` /
`missing`. Applied once, in the content hooks, so Culture, Daily, Search,
Saved, Offline, Journal links and Notifications show the same title.
Readers see one quiet line ("only available in Kyrgyz for now") on actual
fallback; narration never mixes languages.

**Priority order (deterministic):** items linked from Collections, Trails
and Challenges (batch 1, 20 items) → the rest of the Daily pool (31 more,
batch 2) → everything else.

| Content | Total | RU+EN reviewed (after migration) | Kyrgyz-only |
| --- | --- | --- | --- |
| Culture items (long-form) | 230 | **20** (batch 1) | 210 |
| Culture materials | 4 | 0 (translation-ready) | 4 |
| Explore destination facts | 14 destinations | **6** (Chuy, Naryn, Osh, Jalal-Abad, Suusamyr, Arslanbob) | 8 - each contains a claim awaiting verification (§5), deliberately not translated |
| Quest text | 1 | 0 (translation-ready; see Guided Quests) | 1 |

OYNO is **not** fully localized: 210 culture articles, 4 materials, 8
destinations' facts and the legacy quest remain Kyrgyz-only.

## 9. Sources & verification (2026-09-27)

Reused: `culture_items.accuracy_level` + `sources`, `culture_materials`
(same), `discoveries` (same), `explore_regions.status`. New
(`20260927000003_content_verification.sql`, **unapplied**):
`explore_regions.sources` seeded from `content/explore/*.md`; "verified
needs ≥1 source" constraints on all four tables (new writes); admin-only
`content_review_notes`.

Real state: culture items 0 verified / 65 partially / 165 unverified (91
with no sources); explore 11 "verified" with **no sources in the app** →
after the migration, the 9 destinations with claims awaiting review become
"partially verified" (the claims themselves stay open, §5).

App: one quiet "Sources & notes" row at the end of Culture items,
materials and destinations (the top "Unverified" chip is gone) → sheet
with calm wording ("Reviewed against listed sources" / "Some details are
still being reviewed" / "This entry is still being reviewed") and real
sources labelled by publisher, opened in the in-app browser. No review
date (none stored). Challenges show the same wording under explanations
whose source isn't fully reviewed; 10 questions resting on flagged claims
are capped at "partially verified". Daily already excludes unverified
entries. Search ranking unchanged.

## 10. Guided Quests 2.0 (2026-09-27)

4 quests, defined in app code (KG/RU/EN) and mirrored server-side in
`20260927000004_guided_quests.sql` (**not applied yet**): Inside the Boz Uy,
Craft & Ornament, Horse Games, Mountain Journey. Steps use real signals only
(game played, destination visited, lab flag, collection challenge finished);
there is no "read an article" step because no real read signal exists.
`claim_guided_quest` re-checks every step and pays 80 XP / 40 coins once per
account. Guests see progress but no reward (existing economy rule). No
Music & Komuz quest: only one real signal (the lesson) exists today.

## 11. Culture Labs 2.0 (2026-09-27)

Oymo, Shyrdak, Boz Uy and Komuz each show a short "About this lab" note:
what it is, what you do, what is simplified. Completion = the four existing
server flags; rewards stay once-only. Oymo and Shyrdak creations can be
shared as an image card carrying only the design, the lab name and OYNO
branding. Komuz: string taps are visual only; melodies are real recordings;
the inaccurate `Komuz.png` is not used.

## 12. Story Companion 2.0 (2026-09-27)

The player's chosen guide (Бек / Айдана / Аяна; anything else, including
Бөрү / Тулпар / Элчи and unknown server values, falls back to Бек) speaks
one authored line at: Daily intro/outro, Trail start/finish, Challenge
start/result, Journey, the achievement modal, first-use Saved / Journal /
Quests, and quest intro/finish. Game hosts without art (Бөрү, Тулпар,
Элчи) are presented by Бек; their lines are character-neutral.
Presence by age: child/preteen everywhere, teen all but the achievement
modal, adult only quests and empty states. Emotion is fixed per moment; no
haptics; game intro respects Reduce Motion. Analytics:
`companion_moment_shown` with companion id, surface and moment only.
