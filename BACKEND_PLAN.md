...
| **6c** ✅ | `culture_categories`/`culture_materials`/`explore_regions`/`quests` + seed migration from the existing hardcoded TS arrays. **Done and verified live (2026-08-22)** - full writeup in `PROGRESS_AUDIT.md`. Refined during implementation: the `current`/`total` category counts and `discoveredPercent`/`exploreProgress` numbers were explicitly mock in the source TS (never wired to a real count) and were deliberately *not* copied into the database - moving a fake number into Postgres doesn't make it real, it just relocates the fakeness (master prompt §90, §118). Those stay client-side mock constants, unchanged, pending a real collection-items system. Images also stay client-side (bundled `require()` assets - Storage is Phase 6g). |
