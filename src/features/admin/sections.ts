...
export const ADMIN_SECTIONS: AdminSectionConfig[] = [
  ...
  },
  {
    ...
    toParams: (v) => ({
      ...
      p_title_kg: v.title_kg,
      p_title_ru: v.title_ru,
      p_title_en: v.title_en,
      p_xp_reward: toIntOrNull(v.xp_reward) ?? 0,
      p_accuracy_level: v.accuracy_level || 'unverified',
      p_sources: toArray(v.sources),
      p_published: v.published !== 'false',
      p_sort_order: toIntOrNull(v.sort_order) ?? 0,
    }),
  },
  {
    ...
    idField: 'id',
    titleField: 'title_kg',
    fetch: () => fetchTable('quest_steps', 'step_order'),
    ...
    toParams: (v) => ({
      p_id: v.id,
      p_quest_id: v.quest_id,
      p_step_order: toIntOrNull(v.step_order) ?? 1,
      p_step_type: v.step_type,
